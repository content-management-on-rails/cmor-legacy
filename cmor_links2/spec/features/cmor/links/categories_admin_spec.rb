...
feature 'Cmor::Links::Category admin' do
  ...
  def set_resource_path
    @resource_path = 'cmor_links_categories'
  end
  ...
  def fill_new_form
    select I18n.locale.to_s,            from: 'cmor_links_category[locale]'
    fill_in 'cmor_links_category[name]', with: 'Example category'
    select 'textile',                   from: 'cmor_links_category[markup_language]'
  end

  def fill_edit_form
    fill_in 'cmor_links_category[name]', with: 'An updated category'
  end
