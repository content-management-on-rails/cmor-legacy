...
RSpec.describe "/api/cmor-testimonials/testimonials", type: :request do
  let(:base_path) { "/api/cmor-testimonials/testimonials" }
  let(:resource_class) { Cmor::Testimonials::Testimonial }
  let(:factory_name) { resource_class.name.underscore.gsub("/", "_") }
  let(:attribute_keys) { %w[body category_id company created_at fullname position published_at role updated_at] }
  ...
  describe "PUT /:id" do
    ...
    let(:headers) { {"Accept" => "application/json"} }
    let(:params) { { param_key => {body: "New body"}} }
    ...
    describe "when authenticated" do
      ...
      describe "persistence changes" do
        it { expect{ put(update_path, headers: headers, params: params) }.to change{ resource.reload.body }.to("New body") }
      end
    end
  end
