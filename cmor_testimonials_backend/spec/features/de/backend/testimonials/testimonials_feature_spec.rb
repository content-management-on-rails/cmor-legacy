...
RSpec.describe '/de/backend/testimonials/testimonials', type: :feature do
  ...
  describe 'replacing image' do
    ...
    before(:each) do
      visit(edit_path)
      attach_file 'testimonial[overwrite_image][]', Cmor::Testimonials::Engine.root.join(*%w( spec files cmor testimonials testimonials enerbunny.png))
    end
    it { expect{ submit_button.click  }.to change{ resource.image } }
  end
