...
module Cmor
  module Cms
    describe TemplateResolver do
      subject { Cmor::Cms::LayoutResolver.instance }
      ...
      describe "#find_templates" do
        ...
        context "template lookup without format" do
          before(:each) do
            @page = Cmor::Cms::Layout.create! do |template|
              template.pathname = "/"
              template.basename = "foo"
              template.locale = ""
              template.format = ""
      ...
      describe "#initialize_template" do
        ...
        it "returns an ActionView Template" do
          record = Cmor::Cms::Layout.create! do |template|
            template.pathname = "/"
            template.basename = "foo"
            template.format = "html"
            template.handler = "erb"
      ...
      describe "#build_source" do
        ...
        before(:each) do
          @record = Class.new
          @record.singleton_class.define_method(:body=) { |value| @body = value }
          @record.singleton_class.define_method(:body) { @body }

          @record.body = body
        end

        it "should use the body as source" do
          expect(subject.build_source(@record)).to eq(body)
        end
      end
    end
  end
