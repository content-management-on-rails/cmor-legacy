...
RSpec.describe "/api/cmor-carousels/carousels", type: :request do
  ...
  let(:attribute_keys) { %w[created_at identifier locale updated_at variant_options] }
  ...
  describe "PUT /:id" do
    ...
    let(:headers) { {"Accept" => "application/json"} }
    let(:params) { { param_key => {identifier: "new-identifier"}} }
    ...
    describe "when authenticated" do
      ...
      describe "persistence changes" do
        it { expect{ put(update_path, headers: headers, params: params) }.to change{ resource.reload.identifier }.to("new-identifier") }
      end
    end
  end
