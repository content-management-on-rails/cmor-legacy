...
feature 'Cmor::Links::Link admin' do
  ...
  def set_resource_path
    @resource_path = 'cmor_links_links'
  end

  def set_resource_class
    @resource_class = Cmor::Links::Link
  end
  ...
  def fill_new_form
    select "|-- #{@category.name}", from: 'cmor_links_link[cmor_links_category_id]'
    fill_in 'cmor_links_link[name]', with: 'Example link'
    fill_in 'cmor_links_link[url]',  with: 'http://www.example.com'
    select 'textile',               from: 'cmor_links_link[markup_language]'
  end

  def fill_edit_form
    fill_in 'cmor_links_link[name]', with: 'An updated link'
  end
