...
RSpec.describe Cmor::Links::ApplicationViewHelper, type: :view_helper do
  ...
  describe "render_footer" do
    it { expect(html).to have_css(".link-footer") }
  end
  ...
  describe "render_category" do
    ...
    it { expect(html).to have_content(category.name) }
    it { expect(html).to have_css("##{dom_id(category)}") }
  end
  ...
  describe "render_link" do
    ...
    it { expect(html).to have_link(link.name, href: link.url) }
  end
end