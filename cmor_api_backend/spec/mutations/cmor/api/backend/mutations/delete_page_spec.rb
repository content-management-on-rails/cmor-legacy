...
RSpec.describe Cmor::Cms::Backend::Mutations::DeletePage, type: :request do
  describe '.resolve' do
    ...
    let(:query) do
      <<~GQL
        mutation {
          #{mutation_name}(input: { id: "#{resource.to_param}" }) {
            #{type_name} {
    ...
    describe 'destroys the expected resource' do
      it do
        expect do
    ...
    describe 'returns the expected resource' do
