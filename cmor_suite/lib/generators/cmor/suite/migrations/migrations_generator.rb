  ...
  module Suite
    module Generators
      class MigrationsGenerator < Rails::Generators::Base
        ...
        def sub_modules
          @sub_modules ||= %w(blog carousels cms contact files galleries links rbac tags user_area) - options['excluded_modules']
        end

        def needs_active_storage?
          (%w(blog carousels files galleries) & sub_modules).any?
        end
