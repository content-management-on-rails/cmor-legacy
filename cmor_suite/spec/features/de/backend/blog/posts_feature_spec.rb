...
RSpec.describe '/de/backend/blog/posts', type: :feature do
    ...
    it {
      expect(subject).to implement_create_action(self)
        ...
        .within_form('#new_post') {
          ...
          attach_file('post[append_asset_detail_assets][]', Cmor::Blog::Engine.root.join('spec', 'files', 'cmor', 'blog', 'post', 'asset', 'example.jpg'))
        }
  ...
  describe 'appending asset details' do
    let(:existing_asset_details) { create_list(:cmor_blog_asset_detail, 2, post: resource) }
    let(:resource) { create(:cmor_blog_post) }
