...
RSpec.describe Cmor::Carousels::Carousel, type: :model do
  describe "#append_item_detail_assets" do
    ...
    describe "persistence changes" do
      it {
        expect {
          subject.append_item_detail_assets = new_assets
          subject.save!
        }.to change { subject.item_details.count }.from(1).to(2)
      }
    end
  end
  ...
  describe "#overwrite_item_detail_assets" do
    ...
    describe "persistence changes" do
      it {
        expect {
          subject.overwrite_item_detail_assets = new_assets
          subject.save!
        }.to change { subject.item_details.count }.from(2).to(1)
      }
    end
  end
end
